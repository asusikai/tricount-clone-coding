import 'package:supabase_flutter/supabase_flutter.dart';

import '../../data/repositories/group_repository_impl.dart';
import '../../domain/repositories/group_repository.dart';

/// 그룹 관련 서비스 클래스
///
/// 내부적으로 GroupRepository를 사용하여 데이터 접근을 처리합니다.
class GroupService {
  GroupService(this._repository);

  final GroupRepository _repository;

  /// SupabaseClient를 직접 받는 생성자 (하위 호환성)
  GroupService.fromClient(SupabaseClient client)
    : _repository = GroupRepositoryImpl(client);

  /// 그룹 생성
  ///
  /// [name] 그룹 이름
  /// [baseCurrency] 기본 통화 (예: 'KRW', 'USD')
  ///
  /// 반환: 생성된 그룹 ID
  Future<String> createGroup({
    required String name,
    required String baseCurrency,
  }) => _repository.createGroup(name: name, baseCurrency: baseCurrency);

  /// 초대 코드로 그룹 가입
  ///
  /// [inviteCode] 초대 코드 (UUID)
  ///
  /// 반환: 가입한 그룹 ID
  Future<String> joinGroupByInviteCode(String inviteCode) =>
      _repository.joinGroupByInviteCode(inviteCode);

  /// 사용자가 가입한 그룹 목록 조회
  Future<List<Map<String, dynamic>>> getUserGroups() =>
      _repository.getUserGroups();

  /// 그룹 멤버 목록 조회 (사용자 정보 포함)
  Future<List<Map<String, dynamic>>> getGroupMembers(String groupId) =>
      _repository.getGroupMembers(groupId);

  /// 그룹 상세 정보 조회
  Future<Map<String, dynamic>> getGroupDetail(String groupId) =>
      _repository.getGroupDetail(groupId);

  /// 그룹 삭제 (소유자만 가능)
  Future<void> deleteGroup(String groupId) => _repository.deleteGroup(groupId);

  /// 그룹 초대 링크 생성
  ///
  /// [groupId] 그룹 ID
  ///
  /// 반환: 딥링크 URL
  Future<String> getInviteLink(String groupId) =>
      _repository.getInviteLink(groupId);
}
